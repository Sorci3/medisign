"""
MediSign Assist — Real-time sign language inference (TCN) — version normalisée
===============================================================================
Identique à inference.py, avec une normalisation corps-relative appliquée
à chaque frame avant l'inférence.

Normalisation : soustrait le milieu des épaules (landmarks pose 11 et 12),
divise par la largeur inter-épaules. Rend la représentation invariante à la
position et à la distance par rapport à la caméra.

IMPORTANT : ce fichier doit être utilisé avec un modèle entraîné via
            01_tcn_preprocessing_normalized.ipynb + 02_tcn_training.ipynb
            (modèle attendu : src/models/tcn_medisign_normalized.pth)

Usage:
    python src/utils/inference_normalized.py
...
TCN_MODEL_PATH = MODELS_DIR / "tcn_medisign_normalized.pth"
...
def normalize_body_relative(vec: np.ndarray) -> np.ndarray:
    """
    Normalise un vecteur (225,) de keypoints par rapport aux épaules.

    Layout du vecteur : [pose(99) | left_hand(63) | right_hand(63)]
      - pose landmark 11 (épaule gauche)  → indices 33, 34, 35  (x, y, z)
      - pose landmark 12 (épaule droite)  → indices 36, 37, 38  (x, y, z)

    Opération :
      cx, cy = milieu des deux épaules
      scale  = distance inter-épaules (plan xy)
      x_norm = (x - cx) / scale
      y_norm = (y - cy) / scale
      z_norm = z / scale

    Si la pose n'est pas détectée (épaules à zéro) ou si la distance est
    quasi-nulle, le vecteur est retourné tel quel.
    """
    vec = vec.copy()

    sl_x, sl_y = vec[33], vec[34]   # épaule gauche  x, y
    sr_x, sr_y = vec[36], vec[37]   # épaule droite  x, y

    # Pose non détectée → pas de normalisation
    if sl_x == 0.0 and sl_y == 0.0 and sr_x == 0.0 and sr_y == 0.0:
        return vec

    cx    = (sl_x + sr_x) / 2.0
    cy    = (sl_y + sr_y) / 2.0
    scale = float(np.sqrt((sl_x - sr_x) ** 2 + (sl_y - sr_y) ** 2))

    if scale < 1e-6:
        return vec

    # vec[0::3] → toutes les coordonnées x  (indices 0, 3, 6, ..., 222)
    # vec[1::3] → toutes les coordonnées y
    # vec[2::3] → toutes les coordonnées z
    vec[0::3] = (vec[0::3] - cx) / scale
    vec[1::3] = (vec[1::3] - cy) / scale
    vec[2::3] =  vec[2::3]        / scale
...
def extract_keypoints(result: mp_vision.HolisticLandmarkerResult) -> np.ndarray:
    """
    Construit un vecteur (225,) normalisé depuis un HolisticLandmarkerResult.
    Layout : 33 pose × 3 + 21 left_hand × 3 + 21 right_hand × 3 = 225
    Les landmarks manquants sont mis à zéro.
    Les coordonnées sont normalisées corps-relative (épaules).
    """
    ...
    raw = np.concatenate([pose.flatten(), lhand.flatten(), rhand.flatten()])  # (225,)
    return normalize_body_relative(raw)
...
def main():
    ...
    with mp_vision.HolisticLandmarker.create_from_options(options) as landmarker:
        while True:
            ...
            cv2.imshow("MediSign — TCN Inference (normalise)", frame)
